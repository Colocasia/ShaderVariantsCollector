    ...
    public class VariantReferencesWindow : EditorWindow
    {
        ...
        private void DrawToolbar()
        {
            using (new EditorGUILayout.HorizontalScope(EditorStyles.toolbar))
            {
                ...
                {
                    // 强制重建持久化缓存（材料/Prefab索引）
                    EditorUtility.DisplayProgressBar("Rebuilding Cache", "Refreshing material/prefab indexes...", 0.1f);
                    try
                    {
                        SVCCache.RefreshAll();
                    }
        ...
        private void StartScan()
        {
            ...
            EditorApplication.delayCall += () =>
            {
                ...
                {
                    // 自动确保缓存就绪
                    if (!SVCCache.HasIndexes())
                    {
                        EditorUtility.DisplayProgressBar("Building Cache", "Scanning project assets...", 0.1f);
                        SVCCache.EnsureIndexes();
                    }

                    EditorUtility.DisplayProgressBar("Scanning", "Collecting materials (cached)...", 0.4f);
                    var materials = SVCCache.GetMaterialsForVariant(targetVariant);
